...
class EE_API CursorManager {
  public:
	 ...
	 * @param name The name of the cursor
	 */
	virtual Cursor* create( const std::string& path, const Vector2i& hotspot,
  ...
  protected:
	...
	class GlobalCursor {
	  public:
		GlobalCursor() : SysCur( Cursor::SysCursorNone ), Cur( NULL ) {}

		GlobalCursor( Cursor::SysType sysCur, Cursor* cur ) : SysCur( sysCur ), Cur( cur ) {}
