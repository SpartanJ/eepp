...
template <typename T> class tSoundManager {
  public:
	...
	**	@param SampleRate Sample rate (number of samples to play per second) */
	bool loadFromSamples( const T& id, const Int16* Samples, std::size_t SamplesCount,
...
template <typename T>
bool tSoundManager<T>::loadFromSamples( const T& id, const Int16* Samples, std::size_t SamplesCount,
										unsigned int ChannelCount, unsigned int SampleRate ) {
	if ( tSounds.find( id ) == tSounds.end() ) { // if id doesn't exists
		sSound* tSound = &tSounds[id];
...
template <typename T> SoundBuffer& tSoundManager<T>::getBuffer( const T& id ) {
...
template <typename T> Sound& tSoundManager<T>::getFreeSound( const T& id ) {
...
template <typename T> Sound& tSoundManager<T>::operator[]( const T& id ) {
	if ( tSounds.find( id ) != tSounds.end() )
...
template <typename T> Sound* tSoundManager<T>::play( const T& id ) {
	typename std::map<T, sSound>::iterator it = tSounds.find( id );
...
template <typename T> tSoundManager<T>::~tSoundManager() {
...
template <typename T> bool tSoundManager<T>::remove( const T& id ) {
	if ( tSounds.find( id ) != tSounds.end() ) {
		tSounds.erase( id );
		return true;
	}
