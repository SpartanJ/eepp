...
class EE_API ParticleSystem {
  public:
	 ...
	 * @param Pos2 Extended position for some effects to define the expansion over the screen (used
	 * by fire,smoke,snow)
	...
	void create( const ParticleEffect& Effect, const Uint32& NumParticles, const Uint32& TexId,
				 ...
				 const Vector2f& Pos2 = Vector2f( 0, 0 ), const Float& alphaDecay = 0.01f,
	...
	/** Set a callback function for the reset effect of the particles. \n The reset it's where do
	 * you create the effect for every single particle. */
