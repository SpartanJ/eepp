...
either vertex or fragment shader is used, then traditional fixed-function pipeline is used for the
other stage.
*/
class EE_API ShaderProgram {
  public:
	...
	/**  Unbind the program. Anything rendered after Unbind() call will be rendered using the
  ...
  protected:
	...
	/** Constructor that creates a VertexShader from memory and a Fragment Shader from memory, and
	 * link them. */
	ShaderProgram( const char* VertexShaderData, const Uint32& VertexShaderDataSize,
				   const char* FragmentShaderData, const Uint32& FragmentShaderDataSize,
				   const std::string& Name = "" );

	/** Constructor that creates the vertex shader and fragment shader from two files inside a pack
	 */
	ShaderProgram( Pack* Pack, const std::string& VertexShaderPath,
	...
	ShaderProgram( const char** VertexShaderData, const Uint32& NumLinesVS,
				   const char** FragmentShaderData, const Uint32& NumLinesFS,
				   const std::string& Name = "" );
};
