...
App::App() {
	...
	mConfig->load();
	if ( mConfig->pixelDensity > 0 )
		mPixelDensity = mConfig->pixelDensity;
	mFontHinting = mConfig->fontHinting;
	mFontAntialiasing = mConfig->fontAntialiasing;
	mColorScheme = mConfig->colorScheme;
	mUpdateIntervalMs = mConfig->refreshIntervalMs;
}
...
bool App::init() {
	...
	ctx.FrameRateLimit = mConfig->frameRateLimit;
	ctx.Version = mConfig->rendererVersion;
	...
	#settings:hover { tint: var(--primary); }
	]]>
	</style>
	<RelativeLayout id="main_layout" lw="mp" lh="mp">
		<vbox lw="mp" lh="mp">
...
void App::savePreferences() {
	if ( !mConfig->save() )
		Log::error( "Could not save eproc settings to %s", mConfig->getConfigPath() );
}
...
void App::showSettings() {
	...
	mSettingsWindow = SettingsPanel::create( *this );
...
void App::setupUI() {
	...
	auto* settingsBtn = mRoot->find<UIWidget>( "settings" );
	...
	// Connect events
	settingsBtn->on( Event::MouseClick, [this]( const Event* ) { showSettings(); } );
	if ( mEndProcessBtn )
...
void App::setupRefreshTimer() {
	...
	// Publishing only at the sampling cadence would delay the first visible snapshot.
