...
bool onCloseRequestCallback( EE::Window::Window* ) {
	if ( NULL != Editor && Editor->isEdited() ) {
		...
		MsgBox->on( Event::OnConfirm, []( const Event* ) { win->close(); } );
		MsgBox->on( Event::OnClose, []( const Event* ) { MsgBox = NULL; } );
...
EE_MAIN_FUNC int main( int, char*[] ) {
	...
	win = Engine::instance()->createWindow( WindowSettings(
		1280, 720, "eepp - Texture Atlas Editor", WindowStyle::Default, WindowBackend::Default, 32,
	...
	if ( win->isOpen() ) {
...

		{
			...
			UITheme* theme = UITheme::load( "uitheme", "uitheme", "", font.get(),
