...
static te_expr* new_expr( const int type, const te_expr* parameters[] ) {
	...
	const int parameter_count = arity + ( IS_CLOSURE( type ) ? 1 : 0 );
	const int extra_parameter_count = parameter_count > 1 ? parameter_count - 1 : 0;
	const size_t psize = sizeof( void* ) * arity;
	const size_t size = sizeof( te_expr ) + sizeof( void* ) * extra_parameter_count;
