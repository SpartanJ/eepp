	...
	mCb( Cb )
{
	...
	mUIWindow->AddEventListener( cUIEvent::EventOnWindowClose, cb::Make1( this, &UIGOTypeNew::WindowClose ) );
	mUIWindow->Title( "Add GameObject Type" );

	Int32 InitialY		= 16;
	Int32 DistFromTitle	= 18;

	cUITextBox * Txt = mUITheme->CreateTextBox( "GameObject Type Name", mUIWindow->Container(), Sizei(), Vector2i( 16, InitialY ), UI_CONTROL_DEFAULT_FLAGS | UI_DRAW_SHADOW | UI_AUTO_SIZE );
	...
	cUIPushButton * OKButton = mUITheme->CreatePushButton( mUIWindow->Container(), Sizei( 80, 22 ), Vector2i(), UI_CONTROL_DEFAULT_FLAGS_CENTERED | UI_AUTO_SIZE, mUITheme->GetIconByName( "add" ) );
	OKButton->Pos( mUIWindow->Container()->Size().Width() - OKButton->Size().Width() - 4, mUIWindow->Container()->Size().Height() - OKButton->Size().Height() - 4 );
	OKButton->AddEventListener( cUIEvent::EventMouseClick, cb::Make1( this, &UIGOTypeNew::OKClick ) );
	mUIInput->AddEventListener( cUIEvent::EventOnPressEnter, cb::Make1( this, &UIGOTypeNew::OKClick ) );
	...
	CancelButton->AddEventListener( cUIEvent::EventMouseClick, cb::Make1( this, &UIGOTypeNew::CancelClick ) );
...
void UIGOTypeNew::OKClick( const cUIEvent * Event ) {
...
void UIGOTypeNew::CancelClick( const cUIEvent * Event ) {
	mUIWindow->CloseWindow();
}

void UIGOTypeNew::WindowClose( const cUIEvent * Event ) {
	eeDelete( this );
}
