...
UdpSocket::UdpSocket() : Socket( Udp ), mBuffer( MaxDatagramSize ) {}
...
Socket::Status UdpSocket::receive( void* data, std::size_t size, std::size_t& received,
	...
	int sizeReceived = recvfrom( getHandle(), static_cast<char*>( data ), static_cast<int>( size ),
								 0, reinterpret_cast<sockaddr*>( &address ), &addressSize );
