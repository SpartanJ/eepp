...
GameObjectTextureRegionEx::GameObjectTextureRegionEx( const Uint32& Flags, MapLayer * Layer, Graphics::TextureRegion * TextureRegion, const Vector2f& Pos, BlendMode Blend, RenderMode Render, Float Angle, Vector2f Scale, Color Color ) :
...
bool GameObjectTextureRegionEx::isType( const Uint32& type ) {
	return ( GameObjectTextureRegionEx::getType() == type ) ? true : GameObjectTextureRegion::isType( type );
}
...
void GameObjectTextureRegionEx::draw() {
	if ( NULL != mTextureRegion ) {
		...
		mTextureRegion->setDestSize( Sizef( (Float)realSize.getWidth(), (Float)realSize.getHeight() ) );
		...
		if ( mLayer->getMap()->getLightsEnabled() && mLayer->getLightsEnabled() ) {
			...
			if ( MAP_LAYER_TILED == mLayer->getType() ) {
				...
				if ( LM->isByVertex() ) {
					...
					mTextureRegion->draw( mPos.x, mPos.y, *LM->getTileColor( Tile ), mAngle, mScale, mBlend, mRender );
				}
			} else {
				if ( LM->isByVertex() ) {
					mTextureRegion->draw(
					...
					mTextureRegion->draw( mPos.x, mPos.y, LM->getColorFromPos( Vector2f( mPos.x, mPos.y ) ), mAngle, mScale, mBlend, mRender );
				}
			}
		} else {
			if ( NULL != mVertexColors ) {
				mTextureRegion->draw( mPos.x, mPos.y, mAngle, mScale, mVertexColors[0], mVertexColors[1], mVertexColors[2], mVertexColors[4], mBlend, mRender );
			} else {
