...
GameObjectTextureRegion::GameObjectTextureRegion( const Uint32& Flags, MapLayer * Layer, Graphics::TextureRegion * TextureRegion, const Vector2f& Pos ) :
		...
		if ( mLayer->getMap()->getLightsEnabled() && mLayer->getLightsEnabled() ) {
			...
			if ( MAP_LAYER_TILED == mLayer->getType() ) {
				...
				if ( LM->isByVertex() ) {
					...
					mTextureRegion->draw( mPos.x, mPos.y, *LM->getTileColor( Tile ), getRotation(), Vector2f::One, BlendAlpha, getRenderModeFromFlags() );
				}
			} else {
				if ( LM->isByVertex() ) {
					mTextureRegion->draw(
						...
						LM->getColorFromPos( Vector2f( mPos.x + mTextureRegion->getDestSize().y, mPos.y ) ),
						getBlendModeFromFlags(),
						getRenderModeFromFlags()
					);
				} else {
					mTextureRegion->draw( mPos.x, mPos.y, LM->getColorFromPos( Vector2f( mPos.x, mPos.y ) ), getRotation(), Vector2f::One, getBlendModeFromFlags(), getRenderModeFromFlags() );
				}
			}
		} else {
