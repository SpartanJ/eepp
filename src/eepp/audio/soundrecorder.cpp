...
SoundRecorder::~SoundRecorder() {
	...
	assert(!mIsCapturing && "You must call stop() in the destructor of your derived class, so that the recording thread finishes before your object is destroyed.");
}

bool SoundRecorder::start(unsigned int sampleRate) {
	// Check if the device can do audio capture
	if (!isAvailable()) {
		eePRINTL( "Failed to start capture: your system cannot capture audio data (call SoundRecorder::isAvailable to check it)" );
		return false;
	}
	...
	if (!captureDevice) {
		eePRINTL( "Failed to open the audio capture device with the name: %s", mDeviceName.c_str() );
		return false;
	}
...
bool SoundRecorder::setDevice(const std::string& name) {
	...
	if (mIsCapturing) {
		...
		captureDevice = alcCaptureOpenDevice(mDeviceName.c_str(), mSampleRate, format, mSampleRate);
		if (!captureDevice) {
			// Notify derived class
			onStop();
...
void SoundRecorder::setChannelCount(unsigned int channelCount) {
	...
	if (channelCount < 1 || channelCount > 2) {
		eePRINTL( "Unsupported channel count: %d Currently only mono (1) and stereo (2) recording is supported.", channelCount );
		return;
	}
...
bool SoundRecorder::isAvailable() {
	return (Private::AudioDevice::isExtensionSupported("ALC_EXT_CAPTURE") != AL_FALSE) ||
		   (Private::AudioDevice::isExtensionSupported("ALC_EXT_capture") != AL_FALSE); // "bug" in Mac OS X 10.5 and 10.6
}
