...
bool SoundFileWriterOgg::open(const std::string& filename, unsigned int sampleRate, unsigned int channelCount) {
	...
	if (status < 0) {
		eePRINTL( "Failed to write ogg/vorbis file \"%s\" (unsupported bitrate)", filename.c_str() );
		close();
		return false;
	}
	...
	if (status < 0) {
		eePRINTL( "Failed to write ogg/vorbis file \"%s\" (cannot generate the headers)", filename.c_str() );
		close();
		return false;
	}
